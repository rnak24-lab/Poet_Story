...
export default function FeedPage() {
  const { poems, likePoem, unlikePoem, reportPoem, user, viewPoem, blockedUsers, blockUser, seedSamplePoems } = useAppStore();
  ...
  const allPoems = poems.filter(p => !p.isHidden && !blockedUsers.includes(p.authorId));
  ...
  const handleReport = (poemId: string) => {
  ...
  const handleLikeToggle = (poemId: string, likedBy: string[]) => {
    if (!user) return;
    if (likedBy?.includes(user.id)) {
      unlikePoem(poemId, user.id);
    } else {
      likePoem(poemId, user.id);
    }
  };
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="px-6 space-y-4">
        {sorted.map(poem => {
          ...
          return (
            <Link key={poem.id} href={`/poem/${poem.id}`} className="block" onClick={() => viewPoem(poem.id)}>
                ...
                <div className="bg-white px-5 py-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-ink-300">{(poem as any).author || (poem as any).authorName || '익명'} · {flower?.name}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-ink-200">👀 {(poem as any).views || 0}</span>
                    ...
                    {user && poem.authorId !== user.id && (
                      <button onClick={(e) => { e.preventDefault(); if (confirm(`${(poem as any).authorName || '이 사용자'}님을 차단하시겠어요?`)) { blockUser(poem.authorId); } }} className="text-ink-200 hover:text-red-400 text-xs">🚫</button>
                    )}
                  </div>
                </div>
              </div>
