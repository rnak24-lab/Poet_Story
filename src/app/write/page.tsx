...
function FlowerSelectPhase() {
  const { selectFlower, setPhase, authorName, setAuthorName, initQuestionFlow, resetWritingSession, writingDrafts, loadDraft, deleteDraft, user } = useAppStore();
  ...
  return (
    <div className="min-h-screen pb-12">
      <div className="px-6 pt-8 pb-6">
        ...
        <p className="text-ink-400 mt-2 text-sm">오늘은 어떻게 쓸까요?</p>
      </div>
      ...
      <div className="px-6 mb-6">
        <button
          ...
          className={`w-full rounded-2xl p-6 text-left transition-all ${name.trim() ? 'bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 shadow-md hover:shadow-lg hover:border-purple-300 cursor-pointer' : 'bg-gradient-to-br from-purple-50/60 to-pink-50/60 border-2 border-purple-100 opacity-70 cursor-not-allowed'}`}
          style={{ minHeight: '200px' }}
          disabled={!name.trim()}
        >
          <div className="text-4xl mb-3">&#10024;</div>
          <h2 className="text-xl font-bold text-ink-700 mb-1.5">일기처럼 적기</h2>
          <p className="text-sm text-ink-500 leading-relaxed mb-3">
            오늘의 감정·장면을 자유롭게<br/>
            &rarr; AI가 시로 만들어드려요
          </p>
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-50 text-xs text-amber-700 font-medium">
            &#9999;&#65039; 연필 1자루
          </span>
        </button>
        {!name.trim() && (
          <p className="text-xs text-red-400 mt-2 text-center">이름을 먼저 입력해주세요</p>
        )}
      </div>

      <div className="px-6 mb-8">
        <label className="text-sm text-ink-500 font-medium mb-3 block">꽃말을 따라 시 쓰기</label>
...
function AIFromAPhase({ onTryExit }: { onTryExit: (action: () => void) => void }) {
  ...
  const [aiStep, setAiStep] = useState<'pick' | 'reveal' | 'result' | 'edit'>('pick');
  ...
  return (
    <div className="min-h-screen flex flex-col pb-8">
      <div className="px-6 pt-6 pb-3">
        <div className="flex items-center justify-between">
          <button onClick={() => {
            ...
            else if (aiStep === 'reveal') setAiStep('pick');
            else setPhase('part-a-done');
          }} className="text-sm text-ink-300">
            {aiStep === 'edit' ? '← 미리보기로' : aiStep === 'result' ? '← 스타일 선택' : aiStep === 'reveal' ? '← 스타일 선택' : '← 돌아가기'}
          </button>
      ...
      {aiStep === 'pick' && !isGenerating && (
        <div className="px-6 py-2 bubble-fade-0">
      ...
      {/* Style pick */}
      {aiStep === 'pick' && !isGenerating && (
        <div className="px-6 py-2 space-y-4 bubble-fade-1">
      ...
      {/* Reveal — 시 완성! 보러가볼까요? */}
      {aiStep === 'reveal' && !isGenerating && viewingStyle && (
        <div className="flex-1 flex flex-col items-center justify-center px-6 pb-12 bubble-fade-0">
          ...
          <p className="text-xs text-ink-300 text-center leading-relaxed mb-8">
            {flower?.emoji} {flower?.name}의 꽃말을 담아 정성껏 써냈어요.
          </p>
          <button onClick={() => setAiStep('result')}
      ...
      {aiStep === 'edit' && !isGenerating && selectedStyle && (
        <>
          ...
          <div className="px-6 py-3">
            <label className="text-xs text-ink-400 mb-1 block">✨ 시의 제목</label>
            ...
            {!poemTitle.trim() && <p className="text-xs text-warm-500 mt-1">제목을 입력해야 저장할 수 있어요</p>}
          </div>
          ...
          <div className="px-6 space-y-3">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all ${
                isPrivate ? 'bg-purple-50 border-2 border-purple-200' : 'bg-cream-50 border-2 border-transparent hover:border-cream-200'
              }`}
            >
              <div className="flex items-center gap-2.5">
                <span className="text-lg">{isPrivate ? '🔒' : '🌍'}</span>
                <div className="text-left">
                  <p className={`text-sm font-medium ${isPrivate ? 'text-purple-700' : 'text-ink-500'}`}>
                    {isPrivate ? '나만 보기' : '전체 공개'}
                  </p>
                  <p className="text-xs text-ink-300">
                    {isPrivate ? '피드에 표시되지 않아요' : '다른 사람들도 볼 수 있어요'}
                  </p>
                </div>
              </div>
              <div className={`w-11 h-6 rounded-full relative transition-colors ${isPrivate ? 'bg-purple-400' : 'bg-ink-200'}`}>
                <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${isPrivate ? 'translate-x-[22px]' : 'translate-x-0.5'}`} />
              </div>
            </button>
      ...
      {showTitlePrompt && (
        <div className="fixed inset-0 z-50 modal-overlay flex items-center justify-center" onClick={() => setShowTitlePrompt(false)}>
          <div className="bg-white rounded-card w-[90%] max-w-[380px] p-6" onClick={e => e.stopPropagation()}>
            <div className="text-center mb-5">
              <div className="text-4xl mb-3">📝</div>
              <h3 className="font-bold text-ink-700 text-lg">제목을 지어주세요!</h3>
              <p className="text-sm text-ink-400 mt-2 leading-relaxed">
                시에 어울리는 제목을 붙여주세요.<br/>
                제목도 시의 소중한 일부예요.
              </p>
            </div>
            <input
              ...
              placeholder="시의 제목을 입력해주세요"
              className="w-full bg-cream-50 rounded-xl px-4 py-3 text-ink-600 placeholder:text-ink-200 focus:outline-none focus:ring-2 focus:ring-warm-300 mb-4 text-center font-medium"
              autoFocus
            />
            <button
              onClick={() => { if (poemTitle.trim()) { setShowTitlePrompt(false); handleSave(); } }}
              disabled={!poemTitle.trim()}
              className={`w-full py-3 rounded-xl font-medium transition-all ${poemTitle.trim() ? 'bg-ink-700 text-white hover:bg-ink-600' : 'bg-ink-100 text-ink-300 cursor-not-allowed'}`}>
              이 제목으로 저장하기
            </button>
          </div>
        </div>
...
function FinalizePhase({ onTryExit }: { onTryExit: (action: () => void) => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/poems', {
        ...
        body: JSON.stringify({
          ...
          title: poemTitle.trim(), finalPoem, background: poemBackground,
          isCompleted: true, isAutoGenerated: false, isPrivate,
        }),
      });
      if (res.ok) {
        const data = await res.json();
    ...
    if (!savedPoemId) {
      const localPoem: PoemDraft = {
        id: uuidv4(), flowerId: selectedFlowerId || '', authorName, authorId: user?.id || 'anonymous',
        qaItems, sentences, partBText: partCText, partCText: finalPoem, title: poemTitle.trim(), finalPoem,
        background: poemBackground, isCompleted: true, isAutoGenerated: false, isPrivate,
        createdAt: new Date().toISOString(), likes: 0, likedBy: [], views: 0, reports: [], isHidden: false, comments: [],
      };
      addPoem(localPoem);
      savedPoemId = localPoem.id;
...
function FreeWritePhase({ onTryExit }: { onTryExit: (action: () => void) => void }) {
  ...
  const canOpenStyleModal = lengthValid;
  ...
  // Style selected in modal → proceed to pencil confirm
    ...
    <div className="min-h-screen flex flex-col pb-8">
      ...
      {step === 'write' && (
        <div className="px-6 py-2 space-y-4 bubble-fade-0">
          ...
          {/* 변환 버튼 (v3: 스타일은 모달에서 선택) */}
          <button
            onClick={handleConvertClick}
            disabled={!canOpenStyleModal}
            className={`w-full py-4 rounded-2xl font-medium text-lg transition-all ${canOpenStyleModal ? 'bg-ink-700 text-white hover:bg-ink-600' : 'bg-ink-100 text-ink-300 cursor-not-allowed'}`}
          >
            &#9999;&#65039; 시로 변환하기 {canOpenStyleModal && <span className="text-sm opacity-80">(연필 1자루)</span>}
          </button>
        </div>
      )}
      ...
      {showStylePickerModal && (
        <div className="fixed inset-0 z-50 modal-overlay flex items-end justify-center" onClick={() => setShowStylePickerModal(false)}>
          <div className="bg-white rounded-t-[24px] w-full max-w-[500px] p-6 pb-8 animate-slide-up" onClick={e => e.stopPropagation()}>
            <div className="text-center mb-5">
              <h3 className="font-bold text-ink-700 text-lg">시 스타일을 골라주세요</h3>
              <p className="text-sm text-ink-400 mt-1">어떤 분위기의 시로 만들까요?</p>
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowStylePickerModal(false)}
                className="flex-1 py-3 rounded-xl bg-cream-100 text-ink-500 font-medium text-sm">취소</button>
              <button onClick={handleStyleSelected}
                disabled={!selectedStyle}
                className={`flex-1 py-3 rounded-xl font-medium text-sm transition-all ${selectedStyle ? 'bg-ink-700 text-white hover:bg-ink-600' : 'bg-ink-100 text-ink-300 cursor-not-allowed'}`}>
                다음 &rarr;
              </button>
            </div>
          </div>
